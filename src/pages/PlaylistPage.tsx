...
const PlaylistPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (owner === data.sub) {setCanSearch(true); setIsOwner(true)}
      else {setCanSearch(false); setIsOwner(false)}
    };
    fetchData();
  }, [owner]);
  ...
  return (
    <div className="min-h-[100vh] w-full bg-[linear-gradient(180deg,#1574F5_0%,#1453AB_16%,#13458A_35%,#112745_55%,#101721_75%,#101721_100%)]">
      <PlaylistDeatails setOwner={setOwner} search={searchText} setSearch={setSearchText} setReady={setReady} />
