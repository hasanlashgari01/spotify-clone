...
const MusicPlayer: React.FC = () => {
  ...
  return (
    <>
      {/* 🟢 Desktop Player */}
      <motion.div
        className="fixed bottom-4 left-1/2 z-40 hidden w-[95%] -translate-x-1/2 transform items-center justify-between rounded-2xl bg-gray-900/90 p-4 text-white shadow-xl backdrop-blur-md md:flex md:w-4/5"
        ...
        <div className="flex flex-1 flex-col items-center px-4">
          <div className="flex w-full flex-1 flex-col items-center px-4">
            <input
              ...
              style={{
                background: `linear-gradient(to right, #48bb78 ${(position / duration) * 100}%, #2f855a 0%)`,
                transition: 'background 0.2s ease-out', // smooth background transition
              }}
            />
          ...
          >
            ...
            <div className="mt-6 px-6">
              <input
                ...
                value={position}
                onChange={(e) => onSeekChange(Number(e.target.value))}
                className="h-1 w-full cursor-pointer bg-gradient-to-r from-green-400 to-green-600 bg-[length:200%] accent-green-500 transition-all duration-500 ease-in-out"
                style={{
                  backgroundSize: `${(position / duration) * 100}% 100%`, // Make the gradient fill up as the slider moves
                }}
              />
