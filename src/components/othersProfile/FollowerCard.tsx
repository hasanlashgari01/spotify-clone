...
=======
import { useEffect, useState, useRef, useCallback } from 'react';
import {  getUserFollowers, getUserFollowings, Followers, Followings, UserService } from '../../services/userDetailsService';
import { authService } from '../../services/authService';
import FollowerSection from './FollowerSection';
import { XIcon } from 'lucide-react';
import { useFollow } from '../../context/UserFansContext';
...
const FollowersCard = ({ open, onClose, id }: FollowersCardProps) => {
  ...
  const fetchFollowers = useCallback(
    async (p: number) => {
      ...
      setLoading(true);
      try {
        const [res] = await Promise.all([getUserFollowers(String(id), p, limit)]);
        setFFollowers((prev) => {
