...
export const MusicPlayerProvider: React.FC<MusicPlayerProviderProps> = ({
  ...
  const playSong = async (song: Song, nextQueue?: Song[]) => {
    ...
    const setAndPlay = (src: string, useHls = false) => {
      teardownHls();
      if (useHls && Hls.isSupported()) {
        const hls = new Hls({
          ...
          preload: true as unknown as boolean,
        });
        hlsRef.current = hls;
    ...
    const isHls = /\.m3u8($|\?)/i.test(mapped.audioUrl);
    ...
    if (token && !isHls) {
      fetch(mapped.audioUrl, {
        ...
        .catch(() => setAndPlay(mapped.audioUrl));
    } else if (isHls) {
      // HLS stream: use hls.js
      setAndPlay(mapped.audioUrl, true);
    } else {
