...
export const MusicPlayerProvider: React.FC<MusicPlayerProviderProps> = ({ children }) => {
  ...
  const baseApiUrl = useMemo(() => import.meta.env.VITE_BASE_URL || "https://spotify-music.liara.run", []);
  ...
  useEffect(() => {
    try {
      ...
      if (savedTrack) {
        const parsed = JSON.parse(savedTrack) as { track: Track; queue?: Song[]; position?: number };
      ...
      try {
        if (currentTrack) {
          localStorage.setItem(
            "mp_lastTrack",
            JSON.stringify({ track: currentTrack, queue, position: audio.currentTime })
          );
        }
      } catch {}
    };
  ...
  const playSong = (song: Song, nextQueue?: Song[]) => {
    ...
    try {
      localStorage.setItem(
        "mp_lastTrack",
        JSON.stringify({ track: mapped, queue: Array.isArray(nextQueue) ? nextQueue : queue, position: 0 })
      );
    } catch {}
    ...
    const lastUrl = (audio as any)[lastObjectUrlRefKey] as string | undefined;
    if (lastUrl) {
      URL.revokeObjectURL(lastUrl);
      (audio as any)[lastObjectUrlRefKey] = undefined;
    }
    ...
    if (token) {
      fetch(mapped.audioUrl, {
        ...
        .then(async (res) => {
          ...
          (audio as any)[lastObjectUrlRefKey] = objectUrl;
  ...
  }, [handlePlayPause, onSeekChange, handleNext, handlePrev, cycleRepeat, toggleShuffle, position, volume]);
